...
module.exports = {
    data: new discord_js_1.SlashCommandBuilder()
        .setName("list")
        .setDescription("List user's weekly, monthly, or yearly top artists or songs.")
    ...
    async execute(bot, client, interaction, response) {
        ...
        if (length > 30 || length < 0) {
            return response.error("custom", "List size cannot be less than 0 or greater than 30.");
        }
        ...
        if (list_type === "artist") {
            ...
            const embed_list = top_artists
                ...
                return `${artist["@attr"].rank}. **${(0, escapemarkdown_1.default)(artist.name)}** — **${artist.playcount}** plays ${diff_str}`;
            })
                .join("\n");
            const embed = new discord_js_1.EmbedBuilder()
                .setTitle(`${interaction.user.username}'s ${time_text}-top ${list_type}s`)
            ...
            if (time_frame === "overall") {
                await db.log_list_artist(cached_log, interaction.user.id, interaction.guild.id);
            }
            ...
            const embed_list = top_tracks
                .map((track) => {
                return `${track["@attr"].rank}. **${(0, escapemarkdown_1.default)(track.name)}** by **${(0, escapemarkdown_1.default)(track.artist.name)}**— **${track.playcount}** plays`;
            })
                .join("\n");
            const embed = new discord_js_1.EmbedBuilder()
                .setTitle(`${interaction.user.username}'s ${time_text}-top ${list_type}s`)
            ...
            const embed_list = top_albums
                .map((album, i) => {
                return `${i + 1}. **${(0, escapemarkdown_1.default)(album.name)}** by **${(0, escapemarkdown_1.default)(album.artist.name)}**— **${album.playcount}** plays`;
            })
                .join("\n");
            const embed = new discord_js_1.EmbedBuilder()
                .setTitle(`${interaction.user.username}'s ${time_text}-top ${list_type}s`)
